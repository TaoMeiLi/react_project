...
function runserver(port, dir, mock=false) {
	...
	if (mock) {
		app.all('*', function(req, res, next) {  
		    ...
		    res.header("Access-Control-Allow-Origin", `http://${serverConfig.public_host}:${serverConfig.port}`);
	...
	const api = walk(serverConfig.mock_path)
	    ...
	    .map(p=>p.path).forEach(part=>require(part)(app, serverConfig.mock_prefix));
...
runserver(serverConfig.port, './public');
